  ...
  output$distPlot <- renderPlotly({
    ...
    if (location != "NA" && first_start_year != "NA" && first_start_month != "NA") { # if to check to make sure a location/month/year is selected
      ...
      if ((second_end_day == "NA" || second_start_month == "NA" || second_start_year == "NA" || second_location == "NA") && (location != "NA" && first_start_year != "NA" && first_start_month != "NA")) {
        ...
        # gets the graph
        interactive_plot <- single_plot(data, data_maxmin, data_avg, input$dataset, thresholds, paste(input$firstMonth, input$firstYear), input$location)
        ...
        # gets the graph
        interactive_plot <- double_plot(data, data_maxmin, data_avg, second_data_maxmin, second_data_avg, input$dataset, thresholds, paste(input$firstMonth, input$firstYear), input$location, paste(input$secondMonth, input$secondYear), input$secondLocation)
...
single_plot <- function(data, data_maxmin, data_avg, dataset, thresholds, month_year, location) {
  ...
  plot <- ggplot() +
    ...
    geom_point(data = subset(min_thresholds_data, low_flag), aes(x = timestamp, y = daily_min), color = "blue", size = 2.5, shape = 18) +
    geom_point(data = subset(min_thresholds_data, high_flag), aes(x = timestamp, y = daily_min), color = "red", size = 2.5, shape = 18) +
    # geom_point(data = subset(min_thresholds_data, norm_flag), aes(x = timestamp, y = daily_min), color = "black", size = 2, shape = 3) +
    geom_point(data = subset(max_thresholds_data, low_flag), aes(x = timestamp, y = daily_max), color = "blue", size = 2.5, shape = 18) +
    geom_point(data = subset(max_thresholds_data, high_flag), aes(x = timestamp, y = daily_max), color = "red", size = 2.5, shape = 18) +
...
double_plot <- function(data, data_maxmin, data_avg, second_data_maxmin, second_data_avg, dataset, thresholds, month_year_first, location_first, month_year_second, location_second) {
  ...
  plot <- ggplot() +
    ...
    geom_point(data = subset(second_max_thresholds_data, low_flag), aes(x = timestamp, y = daily_max), color = "blue", size = 2.5, shape = 18) +
    geom_point(data = subset(second_max_thresholds_data, high_flag), aes(x = timestamp, y = daily_max), color = "red", size = 2.5, shape = 18) +
...
gauge_chart <- function(wqi, location) {
  ...
  gauge_chart <- gauge_chart %>%
    layout(
      paper_bgcolor = "#333333",
      font = list(color = "white"),
      title = list(text = "WQI", x = 0.55, y = 0.1)
    )
