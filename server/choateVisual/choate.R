...
# Define UI
ui <- fluidPage(
  titlePanel(h1("Choate Monthly Report", align = "center",
                style = "color: white;")),
  plotOutput("distPlot"),
...
server <- function(input, output) {
  output$distPlot <- renderPlot({
    ...
    ggplot(data = data.frame(Date = 1:length(rolling_mean), Value = rolling_mean),
           aes(x = Date, y = Value)) +
      geom_line(color = color, linewidth = 1.5) +
      labs(x = "Date", y = "Value") + 
      theme(plot.background = element_rect(fill = "#333333"),
            panel.background = element_rect(fill = "#333333"),
            panel.border = element_rect(colour = "#444444", fill=NA, size = 2),
