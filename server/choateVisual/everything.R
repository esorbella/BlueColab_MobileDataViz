...
server <- function(input, output) {
  output$distPlot <- renderPlotly({
    ...
    thresholds_data <- data %>%
      mutate(
        low_flag = ifelse(input$dataset < thresholds[[input$dataset]][1], TRUE, FALSE),
        high_flag = ifelse(input$dataset > thresholds[[input$dataset]][2], TRUE, FALSE)
      )
    ...
    plot <- ggplot() +
      ...
      geom_point(data = subset(thresholds_data, low_flag), aes(x = timestamp, y = value), color = "red", size = 2, shape = 6) +
      ...
      labs(title = "Daily Min / Max / Average", x = "Month-Day", y = c(input$dataset)) +
      theme(
