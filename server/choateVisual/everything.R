...
ui <- fluidPage(
  ...
  # initial dropdown for graph type
  selectInput("type", "Choose a graph type:",
    choices = c("Raw Data", "Daily min/max average"),
    selectize = FALSE
  ),
...
server <- function(input, output) {
  output$distPlot <- renderPlot({
    # logic to take in the dropdown
    type <- switch(input$type,
      "Raw Data" = "rd",
      "Daily min/max average" = "dmma"
    )
